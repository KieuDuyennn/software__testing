...
- The inventory captured only 0.6 GB free RAM; official runs require a cleaner
  machine state and must record it.
...
- Capture a visible dxdiag screenshot whose hostname is `KIEUDUYEN`.
- During each official run, show JMeter and Task Manager in the same frame.
- Narrate the unlisted demo video in Vietnamese; AI cannot create this evidence.
