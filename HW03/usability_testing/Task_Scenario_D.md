...
> **Task 1.** *"Imagine you registered for an event on this platform, but when you tried to check
> in on the day, the system said you weren't registered at all. You want this fixed. Use the
> platform to report the problem, however you think is the right way — you can invent any details
> you need (event name, your registration date, etc.)."*
>
...
> submit, or gave up.
>
> **Task 2** *(runs a few minutes after Task 1, once the moderator has quietly resolved the
> request from the admin side)*: *"You submitted that report a little while ago. Go find out
> whether anyone has responded to it, and tell me what they said."*
>
...
those are EMS's own labels, and using them would test nothing about findability).
...
1. **Framing (verbatim):** *"Thanks for helping out. I'm testing this event-support feature, not
...
3. Hand over Task 1. Observe neutrally — no leading hints ("did you see the button at the top?"),
   intervene only if fully stuck. Take structured notes (see `Session_Notes_Template.md`).
...
- `Participants_Table.md` — the 5 (+1 pilot) participants, masked contact info
- `Session_Notes/Session_P1.md` … `Session_P5.md` — structured, timestamped notes per session
- `SUS_Responses.csv` — raw 1–5 answers per participant, scored with
  `python .claude/skills/usability-test-study/scripts/score_sus.py usability_testing/SUS_Responses.csv --instrument sus --markdown`
- `Metrics_Table.md` — task success / time / errors, tabulated across the 5 real sessions
- `Usability_Report_ScenarioD.md` — the final Phase-3 deliverable required by §6 Task 2
