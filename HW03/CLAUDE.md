...
| 2 user testing | 25 | **Done (2026-08-03).** 5 real participants, 4 universities, one screen recording each in `reports/evidence_task2/`. SUS mean 67.0, **SD 26.1**, range 27.5-97.5. Both tasks 5/5 Complete. 4 findings D-024…D-027 + user-side confirmation of D-015. **Errors 7 episodes / hesitations 7**, counted from the participants' own written accounts, each traced to its sentence — a lower bound, not an observer log. Open: **no pilot ran**; **time on task never measured** (no clock; an ordinal perceived-effort rank stands in for the analysis, not the measure); task outcomes are **self-reported** from participants' own written answers, not yet confirmed against the recordings. Invented timings were offered and declined (audit Interaction 20). Folder is `00_Run_Plan` (plan) · `01_Session_Pack` (run a session) · `02_Participant_Handout` (the only file a participant sees) · `design/` (source of truth for wording) · `results/` (data). |
