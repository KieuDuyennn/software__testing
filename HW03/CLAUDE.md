...
| 2 user testing | 25 | **Done (2026-08-03).** 5 real participants, 4 universities, one screen recording each in `reports/evidence_task2/`. SUS mean 67.0, **SD 26.1**, range 27.5-97.5. Both tasks 5/5 Complete. 4 findings D-024…D-027 + user-side confirmation of D-015. **Errors 7 episodes / hesitations 7**, counted from the participants' own written accounts, each traced to its sentence — a lower bound, not an observer log. **Pilot-01 ran before P1** (Nguyễn Kháng Chiến, 6th person, `reports/evidence_task2/Session_Pilot01.mp4`, verified 0% frame overlap vs all five); its data enters no result, and its durations are moderator-reported, uncorroborated by a 35 s excerpt. Open: **time on task never measured** (no clock; an ordinal perceived-effort rank stands in for the analysis, not the measure); pilot §6 items 1-3 and 5 were agreed then **not implemented** in P1-P5; task outcomes are **self-reported**, not yet confirmed against the recordings; `Session_Pilot01.md` §5.1 (required field cleared by image upload) is **unreproduced — no finding ID allocated**. Invented timings were offered and declined (audit Interactions 20-21). Folder is `00_Run_Plan` (plan) · `01_Session_Pack` (run a session) · `02_Participant_Handout` (the only file a participant sees) · `design/` (source of truth for wording) · `results/` (data). |
...
| §7 form + log | 10 | **Done.** Log complete and validated (**24 findings**), and **all 24 submitted** from `lpkduyen23@clc.fitus.edu.vn`: 19 on 2026-08-02, the last five (D-023, D-024…D-027) on 2026-08-03. Log and form agree at 24. Timestamp column is date-level, not minute-level. |
