...
> **To actually run a session, open `docs/usability_testing/01_Session_Pack.md` instead of this
> file.** It is a single time-ordered operational copy — invite, screener, consent, both task
> prompts, the admin-side resolve script, the SUS sheet, the four probes and the after-session
> message — so the moderator scrolls one document instead of opening four. This run plan stays the
> planning document; the four design files remain the source of truth for wording.
...
| --- | --- | --- |
| Method | Moderated think-aloud, in person or screen-shared | `docs/usability_testing/design/Task_Scenario_D.md` |
| Participants | 5 real + 1 pilot, all outside this class | brief §6 Task 2 |
| Screens under test | **D1** `/complaints/new`, **D2** `/complaints`, `/complaints/{id}` | `docs/usability_testing/design/Task_Scenario_D.md` |
...
| Session language | **Vietnamese** — every participant-facing script is in Vietnamese | `docs/usability_testing/design/Recruiting_Kit.md`, `docs/usability_testing/design/SUS_Instrument_VI_EN.md` |
...
table is in `docs/usability_testing/design/Task_Scenario_D.md` §"Screens under test" and repeated in the
...
| --- | --- | --- | --- |
| 1 | ~~**Participant EMS accounts**~~ — **settled** | Sign-up + OTP would eat half a session and tests a screen that is not under study | **Resolved in `docs/usability_testing/design/Moderator_Runsheet.md` §1:** you pre-create **6** accounts (pilot + P1-P5), each with a distinct member code so requests are told apart on the admin side without the buggy search box, and you pre-authenticate each participant's tab at T-15min. You type every password yourself; the participant never sees a login screen. Three sub-points there are marked **VERIFY BEFORE PILOT** — settle them live before the pilot. |
...
| 5 | **Which EMS host is live on the day** | The brief warns the instance moves; a dead URL kills a booked slot | Verify the morning of every session day. See `docs/usability_testing/design/Moderator_Runsheet.md` §1. |

## 3. The path, end to end

```
  PHASE 1 — DESIGN & PREPARE            (mostly done)
    [x] task scenario + success criteria      docs/usability_testing/design/Task_Scenario_D.md
    [x] metrics defined                       docs/usability_testing/results/Metrics_Table.md
    [x] session note template                 docs/usability_testing/results/session_notes/Session_Notes_TEMPLATE.md
    [x] recruiting kit + consent script       docs/usability_testing/design/Recruiting_Kit.md
    [x] SUS instrument (VI/EN) + probes       docs/usability_testing/design/SUS_Instrument_VI_EN.md
    [x] moderator run-sheet                   docs/usability_testing/design/Moderator_Runsheet.md
  ...
  PHASE 3 — ANALYSE & REPORT             (I can do most of this once real data exists)
    [ ] 5 session notes complete            docs/usability_testing/results/session_notes/
    [ ] SUS rows entered + scored           docs/usability_testing/results/SUS_Responses.csv
    [ ] metrics tabulated                   docs/usability_testing/results/Metrics_Table.md
    [ ] observations clustered -> findings, ranked 0-4
    [ ] usability report written            docs/03_Task2_Usability_Report_ScenarioD.md
    [ ] new findings appended D-024+        docs/05_Bug_Usability_Findings_Log.md
...
`docs/usability_testing/results/SUS_Responses.csv`, then, from `HW03/`:
...
Paste the table into `docs/usability_testing/results/Metrics_Table.md`. Report **all five individual
...
Fill the success / time / errors table in `docs/usability_testing/results/Metrics_Table.md` from the five
...
`docs/usability_testing/design/Moderator_Runsheet.md` §7.

### 4.6 Feed the findings back into §7

Genuine defects and usability improvements found in Task 2 are appended to
`docs/05_Bug_Usability_Findings_Log.md` as **D-024 onwards** (D-001…D-023 are taken — D-020…D-022
...
So: the tables in `docs/usability_testing/results/Participants_Table.md`,
`docs/usability_testing/results/Metrics_Table.md`, `docs/usability_testing/results/SUS_Responses.csv` and the five
...
- [ ] 6 real people took part (1 pilot + 5 counted), all outside this class, contact details held
      unmasked privately and **masked** in `docs/usability_testing/results/Participants_Table.md` with
...
- [ ] `docs/usability_testing/results/SUS_Responses.csv` has exactly 5 real rows and the scorer runs clean.
      The file currently holds **only the header row** — the worked `EXAMPLE` row lives in
      `docs/usability_testing/design/SUS_Instrument_VI_EN.md` §5 as a format illustration and must never be
      pasted into the CSV or scored
- [ ] `docs/usability_testing/results/Metrics_Table.md` filled with counts, times, errors and SUS scores
...
- [ ] New findings appended to `docs/05_Bug_Usability_Findings_Log.md` from D-024 and submitted to
...
| `docs/usability_testing/design/Recruiting_Kit.md` | before booking: screener, invite text, consent script |
| `docs/usability_testing/design/Task_Scenario_D.md` | the study design: tasks, success criteria, probes |
| `docs/usability_testing/design/Moderator_Runsheet.md` | open during every session |
| `docs/usability_testing/design/SUS_Instrument_VI_EN.md` | at the end of every session, then for scoring |
| `docs/usability_testing/results/session_notes/Session_Notes_TEMPLATE.md` | copied to `Session_P<n>.md` per session |
| `docs/usability_testing/results/Participants_Table.md` | filled as people are recruited |
| `docs/usability_testing/results/SUS_Responses.csv` | one row per session, immediately after |
| `docs/usability_testing/results/Metrics_Table.md` | after all 5 sessions |
