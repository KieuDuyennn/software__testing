...
| **F4** | **2** — Minor | Usability | **The submission confirmation is too transient to establish trust.** Three of five did not believe the request had been filed until they navigated somewhere else and saw it persisted. The toast disappears before it has done its job. | 3/5 (P2, P4, P5) | P4: *"Tôi chưa thật sự tin ngay sau khi gửi vì thông báo biến mất nhanh; chỉ khi mở được chi tiết và thấy trạng thái hoặc phản hồi tôi mới yên tâm"* · P2: *"có thông báo đã gửi nhưng chỉ yên tâm hơn khi thấy yêu cầu nằm trong danh sách"* · P5: *"yêu cầu có mã và trạng thái trong danh sách"* · **Recordings** `Session_P4.mp4`, `Session_P5.mp4`. **Still** `reports/evidence_task2/D-027_landing_list_no_request_id.jpg` — `/complaints`, the page a user lands on after submitting, carrying no request ID on any row and no confirmation region (live EMS, 2026-08-03). *The toast half is not shown: every post-submit frame in P1, P4 and P5 was searched and none contains the confirmation toast — consistent with the finding, but absence of a frame is not offered as proof of it, so that half rests on the three participants' accounts.* | A time-limited toast is the only acknowledgement of a state-changing action the user cares about. Trust is being carried by *persistent* artefacts (the ID, the status pill) that the confirmation itself never shows. | Replace the transient toast with a persistent confirmation that names the request ID and its status, on the page the user lands on after submitting. Pairs naturally with F2's fix. | **D-027** |
...
| **A screenshot per ranked finding** | **Met** | All five ranked findings carry a still. F2, F3 and F5 use a frame cut from the recording of the participant who hit them. F1 and F4 are not single-frame events — one is movement across pages, the other a toast that has already vanished — so they carry captures of the **product structure that caused them**, taken on the live EMS on 2026-08-03 after the sessions and labelled as such in their own rows: the event page with no route to Support anywhere on it, the avatar dropdown where the only route hides, and the landing list with no request ID on any row. No finding points at an unrelated image, and no capture is presented as showing a participant's session. |
| Log genuine findings through the §7 channel | Met | D-024…D-028 are in `docs/05_Bug_Usability_Findings_Log.md` and were all **submitted to the Google Form on 2026-08-03**, alongside D-023 — **log and form both stand at 25**. See §13 |

**Every row in the table above now reads Met.** The last to close was the screenshot-per-finding
requirement: rather than attach a frame that shows something other than what the finding claims, F1
and F4 were evidenced by capturing the product structure behind them on the live system, and each row
states plainly which half of its claim the capture does and does not carry.
...
| F5 | **D-028** | Usability 3 | 2026-08-03 |
