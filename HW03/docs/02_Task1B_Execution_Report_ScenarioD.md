...
>
> Across all 6 screens (360 cells): 136 were applicable, 108 were executed to a Pass/Fail verdict, and 28 could not be executed this session. Those 28 are mostly blocked by DevTools Network-throttle and offline control, which the available browser-automation tools cannot drive, plus a few destructive or precondition-heavy checks that were not risked on a shared live system holding other students' real data. Each one is listed under "Items not executed" below with its cause and who could run it.
>
> **16 Fail cells map to 12 distinct Fail-cell-derived findings.** D-007 and D-008 each span two or three screens sharing one root cause. **D-001, D-009, D-010 and D-015** were found along the way during exploratory testing rather than tied to one executed Fail cell, giving **16 findings in total**. Every Fail is backed by a screenshot embedded inline below or by DevTools/Network evidence, and nothing in this file is fabricated.
>
...
Checklist version executed: `docs/01_Task1A_Shared_GUI_Checklist.md` **v1.9, 60 items**. The full verification rule and expected behaviour for every Item ID live there; this file records Result, Notes and Evidence only.

**The checklist has since moved to v2.0 (62 items) and this report was not re-run against it.** `IA03-16` (user-task routes reachable from the persistent header) and `IA04-18` (attachment viewer must distinguish a tiny valid image from a failed load) were added on 2026-08-02, after this execution finished. Both are **unexecuted on all six screens** and are deliberately not carried into the tables below as N/A or as inferred passes — an item nobody ran has no result. IA04-18 is worth re-running first when this report is picked up again: it exists precisely because the D-018 misdiagnosis recorded at the end of this file went undetected during the original pass.
...
| D3 | 60 | 35 | 30 | 26 | 4 | 25 | 5 |
| D4 | 60 | 28 | 22 | 20 | 2 | 32 | 6 |
| D5 | 60 | 23 | 18 | 15 | 3 | 37 | 5 |
| D6 | 60 | 9 | 5 | 5 | 0 | 51 | 4 |
| **Total** | **360** | **136** | **112** | **95** | **17** | **224** | **24** |
...
**16 Fail cells total, mapping to 12 distinct Fail-cell-derived findings.** D-007 and D-008 each cover Fail cells across two or three screens sharing one root cause, merged under the findings-log rule that one root cause is one finding. All are backed by a screenshot or by DevTools evidence in `reports/evidence_task1b/`:
...
| IA04-10 | IA-04 | N/A, reason: no bar meter or capacity figure on this screen. | | |
| IA04-11 | IA-04 | Not executed, reason: requires DevTools Network "Offline" mode, same tooling gap as D1/D2/D4/D5/D6; no CDP network-conditions control is exposed by the current browser-automation tool set. **Correction (2026-07-31):** an earlier pass had substituted an unrelated live-caught error here instead of leaving this Not executed, on the reasoning that it was "the closest-matching item". On the same re-verification pass that reverted D6's IA04-11 for the identical reason, this row was reverted too: the substituted observation (a raw backend validation string from a Category-dropdown race condition) is a real, valid defect, but it was never actually produced by forcing offline mode, and D-009/D-015 already establish the precedent that a bug found along the way, not from executing the item it gets attached to, is logged standalone rather than forced into a Fail cell. See D-010 in the findings log. | | |
...
| IA04-11 | D1, D2, D3, D4, D5, D6 | Requires DevTools Network "Offline" mode; same tooling gap as above. A page-level `navigator.onLine` override would not actually cut the network, so is not a valid substitute. | A human tester with DevTools open. |
...
| IA02-13 | D4 | **IA02-08 and IA02-09 were cleared on 2026-08-02** using a purpose-filed draft request (#53) — see their rows above. The unsaved-changes-on-Back case for the Response/Internal-note textareas is still untested. | Whoever picks this report back up next; request #53 is still Pending and is safe to experiment on. |
...
| IA04-12 | D3, D4, D5 | Could not isolate and stopwatch-time a save-triggered toast/banner's on-screen duration with the tools available (screenshots are discrete snapshots, not a continuous timer); the banners were observed to persist well past a few seconds during normal use, which is suggestive but not a measurement. (D1's IA04-12 is **not** in this table: it scored Pass on its announce-to-assistive-tech half, verified via `role="alert"` and a `MutationObserver`; only the same unmeasurable duration sub-check was left open there, and it is noted in that row rather than counted as a not-executed item.) | A human tester with a stopwatch, or an agent with a frame-accurate screen recording tool. |
| IA01-06 | D5 | Forcing a genuine zero-notification empty state on `/notifications` would mean deleting every real notification on the shared admin account, which is destructive well beyond what this check needs and would also destroy the D5 evidence the rest of this screen's rows rest on. Not risked; the non-empty state is not a substitute, since the item is specifically about what renders when there is nothing to render. | A human tester on a freshly-registered account that has not yet received any notification, where the empty state is the natural first state and nothing has to be deleted to reach it. |
| IA03-13 (D5, D6) | D5, D6 | **D3 was cleared on 2026-08-02** — see its row above; the tab+page state survives Back/Forward and the rows-per-page bug turned out not to be needed. D5 still has only one page of data available. D6 was not tested for whether the browser Back button closes the lightbox; time-boxed. | Whoever re-tests once D5 holds more than one page of notifications. |
...
Every `Fail` above has a corresponding row in `docs/05_Bug_Usability_Findings_Log.md`, cross-referenced by Item ID, except the four along-the-way findings noted below. **16 findings in total.** D-001 was carried over from the prior partial-execution session, D-002 through D-013 come from the first full D1-D4 pass, and D-015 through D-019 from this session's extension to D5 and D6. Of these, D-007 and D-008 each span two or three screens sharing one root cause. **Three findings were retracted on live re-verification against EMS on 2026-07-31 (D-013, D-018) or on self-review (D-014); D-016 was narrowed from two overlays to one, and D-010 was moved from a substituted Fail cell to an along-the-way finding, on the same re-verification pass.** All three retired IDs are not reused; see "Live re-verification" below and the retraction notes in `docs/05_Bug_Usability_Findings_Log.md`.
