...
| IA01-06 | D5 | The only genuinely empty state here is zero notifications, and the two non-destructive routes to it were both checked on 2026-08-02 and both closed. `/notifications` has no search box and no filter, so there is no "no matches" case to trigger; and the *Go to page* input carries `max="2"`, so an out-of-range page cannot be requested either. That leaves deleting every real notification on a shared admin account, which is destructive well beyond what this check needs and would also destroy the D5 evidence the rest of this screen's rows rest on. | A human tester on any account that has never received a notification — the empty state is that account's natural first state, so nothing has to be deleted to reach it. Worth doing in the same sitting as the IA01-07 / IA04-11 run below, which already needs a fresh sign-in. |
