...
| **§7**: Findings Log + Google Form | 25 findings logged (17 from Task 1B, 3 from Task 3, 5 from Task 2), deduplicated, severity-rated and `check_findings.py` clean. **All 25 are submitted** from `lpkduyen23@clc.fitus.edu.vn`, the address burned into the Task 3 overlays: 19 on **2026-08-02** and six (D-023, D-024…D-028) on **2026-08-03**. D-028 was raised late on 2026-08-03 from a frame-by-frame review of the P5 recording, after that day's first submission run, and was sent the same day, so **log and form agree at 25**. The one residual imperfection: the `Form-submission timestamp` column records the day rather than the minute, since per-submission clock times were not written down — see the column's own note. |
...
- ***4 = 8/10.*** The aggregated log is complete at 25 findings, script-validated, and carries every column §7 names with developer-reproducible steps on every row. Three findings were retracted before submission rather than left standing. One point held back, named rather than absorbed: the timestamp column records the day rather than the minute, since per-submission clock times were not written down. A TA cross-checking the log against the form finds 25 against 25.
...
| Bug & Usability Findings Log, consistent with the Google Form | `docs/05_Bug_Usability_Findings_Log.md` | 25 findings, **all 25 submitted** |
