...
| **§7**: Findings Log + Google Form | **Done.** 24 findings logged (17 from Task 1B, 3 from Task 3, 4 from Task 2), deduplicated, severity-rated and `check_findings.py` clean — and **all 24 submitted to the Google Form** from `lpkduyen23@clc.fitus.edu.vn`, the address burned into the Task 3 overlays: 19 on **2026-08-02**, the remaining five (D-023 and D-024…D-027) on **2026-08-03**. **Log and form agree at 24 each.** The one residual imperfection: the `Form-submission timestamp` column records the day rather than the minute, since per-submission clock times were not written down — see the column's own note. |
...
| 2 | Task 2: User testing with 5 real users (scenario + 5 sessions + analysis → Usability Report) | 25 | 23 |
...
- ***4 = 9/10.*** The aggregated log is complete at 24 findings, script-validated, and carries every column §7 names with developer-reproducible steps on every row — and **all 24 are submitted**, so log and form agree at 24 if the TA cross-checks. Three findings were retracted before submission rather than left standing. The single point held back is that the timestamp column records the day rather than the minute, since per-submission clock times were not written down as the submissions were sent.
...
| Bug & Usability Findings Log, consistent with the Google Form | `docs/05_Bug_Usability_Findings_Log.md` | Done — 24 findings, **all 24 submitted**; log and form agree |
