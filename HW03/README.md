...
| **Findings** | **25 logged** (1 Critical, 8 Major, 6 Minor, 1 Trivial, 9 usability), each reproducible from the steps as written |
| **Evidence** | 32 Task 1B captures + 26 Task 3 captures + 6 session recordings (5 counted + pilot) + 14 survey captures, all from the live system |
...
│   ├── 05_Bug_Usability_Findings_Log.md        ← §7 aggregated findings log (25 findings; 3 retracted on review)
│   ├── 06_AI_Audit_Report.md                   ← §10 mandatory appendix (21 AI interactions, prompts, human review)
...
| **2**: User testing, 5 real users | **Done (2026-08-03).** Five real participants from four universities — medicine, mechanical engineering (×2), food technology, marketing — none enrolled in this course and none working in software; masked contacts in `docs/usability_testing/results/Participants_Table.md`, one screen recording each in `reports/evidence_task2/`, and a **pilot with a sixth person before P1**. Both tasks completed 5/5, yet the study still produced **five ranked findings (D-024…D-028)** plus independent user-side confirmation of D-015, because success rate is not where the signal is: SUS mean **67.0** but **SD 26.1** across **27.5–97.5**, and time on task ran **2:58–8:30** on T1 and **0:22–3:40** on T2 — a 10× spread on reading a reply someone sent you. Recommendations are prioritised, and the top two collapse into one small fix (redirect to the new request's detail page). **Seven error episodes and seven hesitations are counted**, each traced back to the sentence the participant wrote about their own session, and the seven split 5 findability errors against 2 blocked submissions — the second pair being the form working, not failing. **A pilot ran before P1** — a sixth person, `Session_Pilot01.mp4`, checked frame-by-frame against all five counted recordings (0 % overlap) before being relied on, with its data kept out of every metric. The gap stated in Limitations rather than smoothed over is **time on task** (no clock ran; an ordinal perceived-effort rank carries the analysis and is never called a duration) — and, honestly, that the pilot *identified* the missing clock convention and P1–P5 then did not implement it. **Task outcomes and both counts are self-reported** from participants' own written answers rather than moderator-observed — so the counts are a floor, and the recordings that could confirm them have not yet been reviewed for that. **Scoped to D1-D2**, not the D5/D6 extension and not Task 3's D1-D4: `docs/usability_testing/design/Task_Scenario_D.md` has the participant complete the filing half (D1/D2) while the moderator operates D3/D4 out of view, a deliberate scope narrower than Task 1B's, not an oversight. |
...
| **§7**: Findings Log + Google Form | 25 findings logged (17 from Task 1B, 3 from Task 3, 5 from Task 2), deduplicated, severity-rated and `check_findings.py` clean. **24 of the 25 are submitted** from `lpkduyen23@clc.fitus.edu.vn`, the address burned into the Task 3 overlays: 19 on **2026-08-02** and five (D-023, D-024…D-027) on **2026-08-03**. **D-028 is outstanding** — it was raised on 2026-08-03 from a frame-by-frame review of the P5 recording, after the last submission run, so the log stands at 25 and the form at 24 until it is sent. The one residual imperfection: the `Form-submission timestamp` column records the day rather than the minute, since per-submission clock times were not written down — see the column's own note. |
...
- **Google Form submissions:** **24 of 24.** 19 sent 2026-08-02; D-023 and D-024…D-027 sent 2026-08-03. Log and form agree at 24.
- **Demo videos:** TODO.
...
| 2 | Task 2: User testing with 5 real users (scenario + 5 sessions + analysis → Usability Report) | 25 | 24 |
...
- ***2 = 24/25.*** Five real participants from four universities, all outside this course, run 2026-08-03, **plus a pilot with a sixth person before P1**, each with **two independent records**: a completed questionnaire and a screen recording. The recordings were verified genuinely distinct before being relied on — **0 % frame overlap between every pair, and 0 % between the pilot and all five** — and each shows that participant writing their own scenario. Both tasks completed 5/5, and the study still produced five ranked findings (**D-024…D-028**) plus independent user-side confirmation of D-015, because the measures behind the success rate disagree with it: SUS mean 67.0 with **SD 26.1** (range 27.5–97.5), and measured time on task running **2:58–8:30** on T1 and **0:22–3:40** on T2. The tasks are achievable; the path is not discoverable. Five separate orderings — T1 time, T2 time, error count, SUS, and a perceived-effort rank derived from the probe answers before the timing log was transcribed — place the five participants in the same sequence, and two of those five are moderator-recorded while three are self-report. Two findings carry a still cut from the recording of the participant who hit them. Recommendations are prioritised and the top two collapse into one small fix. §11b of the report audits every §6 requirement line by line. **Error and hesitation counts were recovered** — 7 episodes and 7 hesitations, each printed next to the participant sentence it is counted from, labelled a lower bound because a participant cannot report an error they never noticed. Two points are held back for what genuinely is not there: **time on task was never measured**, and the counts are participant-reported rather than observer-recorded. Both needed something to happen during the sessions and cannot be supplied afterwards without inventing data — the pilot flagged exactly this and its timing convention was not carried through to P1–P5, which the report says in its own words rather than leaving for a marker to find.
...
- ***4 = 8/10.*** The aggregated log is complete at 25 findings, script-validated, and carries every column §7 names with developer-reproducible steps on every row. Three findings were retracted before submission rather than left standing. Two points held back, both named rather than absorbed: **D-028 is logged but not yet on the form**, so a TA cross-check finds 25 against 24 until it is sent; and the timestamp column records the day rather than the minute, since per-submission clock times were not written down.
...
`23127184_HW03_AI_GUIUsability_EMS_090.zip`. **Recompute the number and rename the file if the
...
| Bug & Usability Findings Log, consistent with the Google Form | `docs/05_Bug_Usability_Findings_Log.md` | 25 findings, **24 submitted**; **D-028 outstanding** |
