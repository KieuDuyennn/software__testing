...
> pass covered by this file up to Prompt 4 below, v1.3 to v1.7 (Prompts 5-10) reduced and corrected
...
> completeness check. Full detail for v1.3 to v1.7 is in `../AI_Audit_Report.md` Interactions 5-10;
...
| 6 | Assignment specification                                               | Assignment spec     | §4 SUT (EMS feature pools A to D; IA-01 to IA-04 definitions, used as the conformance map in the checklist) · §5 suggested screens · §6 Task 1 Part A requirements                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Local file:`HW03/requirments/2026.HW03.GUI Usability EMS_En.md` (+ `_Vi.md`)                       |
...
| `HW03/refs/Testing GUI Applications.pdf`                                                | The file could not be opened during the v1.2 audit (`PdfReadError: EOF marker not found`; the PDF appears truncated). Nothing was cited from it because nothing could be read from it. Re-downloading it is tracked as §5 action 4.                                                                                                     |
...
The assignment (§2) requires guiding the AI through each step of the technique rather than issuing one generic prompt. The chain below is the complete record: every prompt that shaped the checklist, in order. It stays consistent with `../AI_Audit_Report.md`.

> **How these prompts are recorded.** The working language of these sessions was mixed Vietnamese and
> English, and the raw messages were short and elliptical, continuations of a train of thought
> rather than self-contained instructions. Recording them verbatim would document the typing, not
> the instruction. **Each prompt below is therefore a normalised English rendering of what was
> actually asked**, written out in full so the scope, the constraints and the standard the output was
> held to are all visible. Nothing has been added that was not asked for, and no prompt appears here
> for an exchange that did not happen. The same convention, and the same disclosure, applies in
> `../AI_Audit_Report.md` §1.
...
*Updated 2026-07-31 against the current repository state; items resolved since the v1.2 pass are
marked done rather than left to look outstanding. Task 1B is now complete (see
`../task1b_execution/Execution_Report_ScenarioD.md`), which closes action 5 and changes nothing
about actions 4, 6 and 7.*
...
| 3 | Record the group's review outcome for the v1.2 to v1.7 revision passes | Group | **Done for the student's own review** 2026-07-31: every prompt in §3 above and every interaction in `../AI_Audit_Report.md` §2 now carries a written review outcome stating what was checked, what was accepted and what was rejected (including the defect verdicts stripped from the shared artefact in Interaction 10). Formal sign-off from the other three members is still pending and is tracked separately as action 7. |
| 4 | Re-download `HW03/refs/Testing GUI Applications.pdf`, truncated, unreadable | Group | Still open. Nothing cites it; low priority. |
| 5 | Distribute checklist v1.9 to all four members before Task 1B execution | Group | **Done for this member**: scenario D was executed against v1.9 in full (60 items across 6 screens, `../task1b_execution/Execution_Report_ScenarioD.md`). Whether A, B and C executed against the same version is not verifiable from this repository; confirm with the group before the group-level §7 cross-check. |
