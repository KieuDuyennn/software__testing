...
confirmed by the student and is reported separately so it does not obscure
compliance with the one-per-graded-pool rule.

| API | Pool | Requirement | Endpoint | Role in submission |
|---|---|---|---|---|
| API 1 | A | FR-01 Account registration | `POST /api/register` | Graded Pool A |
| API 2 | D | FR-06 Product detail | `GET /api/products/:id` | Additional coverage |
...
Pool-D coverage and is not part of this three-API uniqueness combination.
