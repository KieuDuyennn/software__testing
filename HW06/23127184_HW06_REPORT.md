...
Demonstration video (Section 7 of the brief):
<https://youtu.be/zgGwFg2e8UE>. It runs the skill end to end on one API,
`POST /api/register`: activating the skill, parsing the specification and FR-01
into an oracle, generating the domain, state, security and schema groups,
stopping at the human-review checkpoint, emitting the Postman collection, then
executing the generated collection with Newman and opening the resulting
report.

The diagram below is my own drawing. I laid it out in draw.io from the design
notes and the pseudocode, deciding myself which stages deserve their own box,
where the two quality gates belong, and which arrows carry a payload versus a
rejection. The three dotted arrows are the point of the drawing: a structural
reject returns to the producing stage, an INVALID or INCOMPLETE review verdict
returns to the stage that produced the case, and a runtime mismatch in the dry
run goes to bug triage instead of silently weakening the oracle. The student ID
is on the input box and on the renderer, because the generator injects and then
asserts `X-Student-Id: 23127184` on every case it emits. The full-size image is
`docs/design/diagram/generator-design.png`, and the editable draw.io source it
was exported from is `docs/design/diagram/generator-design.drawio`.

![AI-driven API test generator pipeline, drawn in draw.io](docs/design/diagram/generator-design.png)
...
The standalone Markdown version remains at `docs/AI_CRITIQUE.md`.
...
2026-08-24. The self-drawn generator diagram was drawn on 2026-08-30 and its
editable draw.io source is checked in beside the exported image. The Newman
evidence cards attached to Issues #26, #33, #47-#55 and #59-#64 are renderings
of fields taken verbatim from the retained pre-fix transcript, produced by
`scripts/build-issue-evidence.py`; each card names that transcript in its
footer, and `evidence/EVIDENCE_INDEX.md` marks which images are screen captures
and which are rendered cards.
