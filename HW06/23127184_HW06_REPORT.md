...
The signed-in Postman workspace now contains all four HW06 collections and the
`EShop - Local (23127184)` environment; both views are captured under
`evidence/postman-cloud/`. Collection Runner, Monitor, Mock Server and the
mandatory executed Console view remain device/user evidence because no Postman
Desktop Agent process was running during capture.
...
group uniqueness confirmation, executed Postman Console/Runner, Monitor/Mock
evidence, self-drawn diagram, and narrated YouTube video.
