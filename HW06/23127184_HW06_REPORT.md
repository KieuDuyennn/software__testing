...
(Pool C). API 2 is additional Pool-A coverage and is reported separately so it
does not obscure compliance with the one-per-pool rule.
...
**Group uniqueness:** pending the student's dated confirmation with group
members. This cannot be inferred from the repository and must be added before
submission.
...
| Gate run log | `evidence/newman-console/suite_gate_20260823-223753.log` |
...
that the current SUT satisfies and passed **1,271/1,271 assertions**.
...
only returns the wrong status but actually changes the order to `canceled`.
...
Workspace, Collection Runner screenshots, Monitor, Mock Server, and Postman
Console screenshot remain cloud/user evidence. Their exact capture steps are in
`docs/postman/POSTMAN_FEATURES.md`; they are not claimed as completed here.
...
1,271/1,271. GitHub Actions still requires two authentic pushed runs: one green
and one red with a genuine defect-revealing case added to the gate. Commit SHA,
run URLs, and screenshots must come from GitHub and are intentionally not
fabricated. See `docs/cicd/CI_CD_REPORT.md`.
...
Real GitHub Issue URLs are recorded in the bug report. Their screenshots still
need to be captured and placed in the evidence folder; the report never invents
external evidence.

## 12. AI-driven test generator (G9.5)

The generator uses a normalized endpoint model, four coverage passes, a
human-review queue, executable rendering, a dry-run validator, and a feedback
loop. Design and pseudocode are in `docs/design/`. A reusable implementation is
under `.claude/skills/api-test-generator/`.

The assignment explicitly requires the diagram to be self-drawn. The repository
therefore supplies a precise drawing brief but does not claim an AI-generated
image as student evidence. The student must draw/export the final PNG and record
the narrated demonstration video.

## 13. AI Critique

See `docs/AI_CRITIQUE.md` (258 words). It identifies the duplicated case,
misattributed security failure, and missing post-condition oracles, and derives
the rule that AI output is a review queue rather than evidence.
...
following are intentionally pending because they must be attributable to the
student: group uniqueness confirmation, Postman Console/cloud screenshots,
GitHub Issue and Actions URLs, self-drawn diagram, and narrated YouTube video.
