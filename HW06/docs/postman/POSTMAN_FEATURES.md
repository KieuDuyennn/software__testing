...
| 2 | **Collections** | ✅ Used | Four generated and executed collections in `postman/collections/`, one per API |
...
| 6 | **Globals** | ✅ Configured | `postman/config/hw06.postman_globals.json` |
...
| 9 | **Test scripts** | ✅ Used | Full run and regression gate: 1,802 assertions each |
...
| 15 | **Data-driven runs (CSV)** | ✅ Used | Four dedicated collections replay one request per CSV row via `pm.iterationData`; `npm run ddt:all` executed 27 iterations and 128 assertions, all passing (`reports/*_ddt.html`, `evidence/newman-console/suite_ddt_20260824-154226.log`) |
...
| 18 | **`--folder` selective runs** | ✅ Working | The CI green gate, driven by `postman/config/ci-suite.json` |
...
| 22 | **Collection documentation** | ✅ Used | Every collection, folder and request carries a description; the request descriptions render the case's dimension, partition, requirement and audit label |
| 23 | **Collection variables** | — | Not used; run-time values are held in the environment instead, so one collection can run against local, CI or mock |
...
Legend: ✅ used · ◐ partially used · — deliberately not used

---

## Mock server contract comparison

The API2 mock returns what the specification promises; the running SUT does
not. Running the same schema-validation folder against both makes the mismatch
...
|---|---|---|---:|---|
| `API1_FR01_Register_ddt` | `POST /api/register` | `postman/data/api1_fr01_register.csv` | 10 | name, email, password partitions |
| `API2_FR06_ProductDetail_ddt` | `GET /api/products/{{ddt_product_id}}` | `postman/data/api2_fr06_product_detail.csv` | 8 | the `:id` partition, incl. a SEC-05 payload |
| `API3_FR11_OrderHistory_ddt` | `GET /api/orders/{{ddt_target}}` | `postman/data/api3_fr11_order_history.csv` | 5 | actor token and target (own history vs. another user's order) |
| `API4_FR13_AdminOrders_ddt` | `GET /api/admin/orders` | `postman/data/api4_fr13_admin_orders.csv` | 4 | the SEC-03 role matrix |

```powershell
npm run ddt:rebuild     # regenerate the four collections from postman/data/
...
in `evidence/newman-console/suite_ddt_20260824-154226.log`.
