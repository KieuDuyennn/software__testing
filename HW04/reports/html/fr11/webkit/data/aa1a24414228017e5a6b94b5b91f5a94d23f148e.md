...
Locator:  getByRole('table').locator('tbody tr').filter({ has: getByText('#20', { exact: true }) }).locator('span')
Expected: "Đã hủy"
Received: "Đã giao"
Timeout:  10000ms
...
Call log:
  ...
  - waiting for getByRole('table').locator('tbody tr').filter({ has: getByText('#20', { exact: true }) }).locator('span')
