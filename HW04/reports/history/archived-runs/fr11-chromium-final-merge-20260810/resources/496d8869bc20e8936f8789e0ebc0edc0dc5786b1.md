...
- Name: fr11_order_history\fr11.order-history.spec.ts >> FR-11 Order history view >> FR11-TC-22 [negative] a canceled order must not be transitionable to delivered
- Location: automation\tests\fr11_order_history\fr11.order-history.spec.ts:320:5

# Error details

```
Error: FR11-TC-22 set status: http://localhost:3000/api/admin/orders/28/status answered 200, expected one of [400]. Body: {"message":"Order status updated"}
...
```
Error: FR11-TC-22: status was changed to "delivered" despite the refusal
