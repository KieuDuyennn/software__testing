...
- Name: fr01_account_registration\fr01.registration.spec.ts >> FR-01 Account registration >> FR01-TC-18 [edge] full name is whitespace only
...
```
Error: the message must name the name field, otherwise the user cannot act on it

expect(locator).toHaveText(expected) failed

Locator: locator('div.bg-red-100')
Expected pattern: /họ tên|tên/i
Timeout: 10000ms
Error: element(s) not found

Call log:
  - the message must name the name field, otherwise the user cannot act on it with timeout 10000ms
  - waiting for locator('div.bg-red-100')
...
```
Error: registration was refused, so fr01.fr01-tc-18.msn1j4inb70@example.com must not be a usable account
