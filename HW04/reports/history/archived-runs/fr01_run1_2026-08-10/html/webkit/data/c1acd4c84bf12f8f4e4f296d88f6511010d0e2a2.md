...
- Name: fr01_account_registration\fr01.registration.spec.ts >> FR-01 Account registration >> FR01-TC-24 [negative] password holding a space and no special character
...
```
Error: the message must name the password field, otherwise the user cannot act on it

expect(locator).toHaveText(expected) failed

Locator: locator('div.bg-red-100')
Expected pattern: /mật khẩu/i
Timeout: 10000ms
Error: element(s) not found

Call log:
  - the message must name the password field, otherwise the user cannot act on it with timeout 10000ms
  - waiting for locator('div.bg-red-100')
...
```
Error: registration was refused, so fr01.fr01-tc-24.msn0m5kv6xk@example.com must not be a usable account
