...
- Name: fr11_order_history\fr11.order-history.spec.ts >> FR-11 Order history view >> FR11-TC-03 [negative] one user must not be able to read another user's order by ID
- Location: automation\tests\fr11_order_history\fr11.order-history.spec.ts:320:5

# Error details

```
Error: FR11-TC-03 order detail: http://localhost:3000/api/orders/23 answered 200, expected one of [401, 403, 404]. Body: {"id":23,"user_id":3,"total_amount":2000000,"status":"pending","shipping_address":"227 Nguyen Van Cu, Q5, TP.HCM","created_at":"2026-08-10 11:36:36"}
...
```
Error: FR11-TC-03: the request was refused-by-requirement yet the order record was returned in full - fields: id, user_id, total_amount, status, shipping_address, created_at

expect(received).not.toHaveProperty(path)

Expected path: not "total_amount"

Received value: 2000000
```
...
```ts
  289 | 
  290 | test.describe('FR-11 Order history view', () => {
  291 |   let ownerToken = '';
  292 |   let adminToken = '';
  293 |   let otherToken = '';
  294 |   let emptyToken = '';
